  ...
  incoherencies: z
    .array(
      z.object({
        ...
        rule: z
          .string()
          .describe("ID of the violated validation rule (e.g. M4, M8, M9)"),
        suggestion: z
          .string()
          .describe(
    ...
    .describe("List of semantic incoherencies found in the configuration"),
});

export type SemanticIssue = z.infer<
  typeof semanticIssueSchema
>["incoherencies"][number];
...
type IssuesTreeNode = Record<
  string,
  IssueEntry[] | Record<string, IssueEntry[]>
>;
...
function buildRulesReference(ruleIds: RuleId[]): Array<{
  rule: string;
  name: string;
